...
namespace uut
{
	Graphics::Graphics(Render* render)
		: _render(render)
	{
		_geometry = new Geometry(_render);
	}

	void Graphics::DrawLine(const Vertex& start, const Vertex& end)
	{
		TestBatch(PRIMITIVE_LINELIST, 2);
	...
	{
		TestBatch(PRIMITIVE_TRIANGLELIST, 3);
		const int count = _verts.Count();
		_verts << v0 << v1 << v2;
	...
	{
		TestBatch(PRIMITIVE_TRIANGLELIST, 4);
		const int count = _verts.Count();
		_verts << v0 << v1 << v2 << v3;
	...
	void Graphics::TestBatch(EPrimitiveType type, int count)
	{
		if (!_verts.IsEmpty() && !_primitive != type)
		{
			Flush();
			return;
		}
